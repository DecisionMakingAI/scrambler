...
This block was lifted from the main donkeycar manual and annotated with the necessary changes to get the software to install.
...
*The operating system flashed was r32.3.1.*

*Setup the following:*

```text
    machine name: scrambler
    user name/password: donkey/donkey
```

To check the software installed, ran the following command with the subsequent output:

```text
donkey@scrambler:~$ hostnamectl
   Static hostname: scrambler
         Icon name: computer
        Machine ID: ca67ded01f62474199a2c58cfd607d5f
           Boot ID: 81993f68dfcb47189da8918825844916
...
*This didn't, so we'll find a better procedure when this is needed.*

##  Step 3: Setup Virtual Env

*The following code was changed, since it exposed a bug when installing the donkeycar code.*
```bash
...
The two important differences were made to the installation.

1. Install virtualenv in system space as opposed to user space.  Having this is user space was one precondition for the __legacy__ error when installing the donkeycar system.
2. Added virtualenvwrapper, as it provides a nicer interface.

[Here](https://docs.python-guide.org/dev/virtualenvs/) is a reference to using this environment.

*This is the code executed.*

```bash
# This command actually upgraded pip from what was there
python3 -m pip install -U pip

# sudo gives it permission to write into system space
...
# make a virtual environment for cv
mkvirtualenv cv
```
...
*This image did not have openCV in it.*
...
```
# enter our virtual environment again
workon cv
```
...
*For good measure, we'll do a reboot here.*
```bash
sudo reboot
```
...
*Note: changed last line above to remove 'opencv' as it caused an error.*
...
*Note: We're running full power so we can issue a -j4.*
...
```
# Rename
sudo mv cv2.cpython-36m-xxx-linux-gnu.so cv2.so
# Go to your virtual environments site-packages folder
...
*Create a cloned workspace to build the donkeycar environment from.*
```bash
pip freeze >requirements.txt
mkvirtualenv -r requirements.txt car

# patch in the cv2 library
cd ~/.virtualenvs/car/lib/python3.6/site-packages/
ln -s /usr/local/lib/python3.6/site-packages/cv2/python-3.6/cv2.so cv2.so
cd ~/projects

# Set this environment to be entered on login
echo "workon car" >>~/.bashrc
```
